#include "../view/ComplexPolyMeshGL.h"
void AnimatedInstancedPolyMesh::bindUniforms( InstanceInfo const &stat ) const
{
...
AnimatedInstancedPolyMesh::AnimatedInstancedPolyMesh( AnimatedInstancedPolyMesh &&mesh )
{
	*this = std::move( mesh );
}
void AnimatedInstancedPolyMesh::operator=( AnimatedInstancedPolyMesh &&mesh )
{
	release();
...
AnimatedInstancedPolyMesh::AnimatedInstancedPolyMesh( std::unique_ptr< Polymesh > &&mesh )
{
...
void AnimatedInstancedPolyMesh::init( std::unique_ptr< Polymesh > &&mesh )
{
	*this = std::move( AnimatedInstancedPolyMesh( std::move( mesh ) ) );
	init();
}
void AnimatedInstancedPolyMesh::init()
{
	if( !__mesh ) return;
	if( isInited() ) return;
	setInited( true );
...
void AnimatedInstancedPolyMesh::release()
{
	if( !isInited() ) return;
	setInited( false );
